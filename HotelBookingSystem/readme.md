...
- POST `/` — Create a room type (request: `RoomTypeCreate`, response: `RoomTypeResponse`) (201)
  - Permission: requires room_service.WRITE (permission dependency)
- GET `/` — List room types (query: `include_deleted: bool`) (response: list[`RoomTypeResponse`])
- GET `/{room_type_id}` — Get a single room type
- PUT `/{room_type_id}` — Update a room type (permission: room_service.WRITE)
- DELETE `/{room_type_id}` — Soft-delete a room type (permission: room_service.WRITE)

### Rooms
Base path: `/api/rooms`

- POST `/` — Create a room (request: `RoomCreate`, response: `RoomResponse`) (201)
  - Permission: requires both BOOKING.WRITE and room_service.WRITE
- GET `/` — List rooms (query filters: `room_type_id`, `status_filter`, `is_freezed`)
- GET `/{room_id}` — Get room details (response: `RoomResponse`)
- PUT `/{room_id}` — Update a room (permission: BOOKING.WRITE & room_service.WRITE)
- PATCH `/{room_id}/status` — Change room status (permission: BOOKING.WRITE & room_service.WRITE)
- DELETE `/{room_id}` — Delete a room (permission: BOOKING.WRITE & room_service.WRITE)

### Amenities
Base path: `/api/amenities`

- POST `/` — Create an amenity (request: `AmenityCreate`, response: `AmenityResponse`) (201)
  - Permission: room_service.WRITE
- GET `/` — List amenities
- GET `/{amenity_id}` — Get amenity
- DELETE `/{amenity_id}` — Delete amenity (permission: room_service.WRITE)
...
- DELETE `/` — Unmap amenity (query: room_id, amenity_id) (permission: room_service.WRITE)

### Room Images
Base path: `/api/rooms/{room_id}/images`

- POST `/` — Upload image for room (multipart/form-data: file, caption, is_primary) (201)
  - Permission: room_service.WRITE (uploader must be authenticated)
- GET `/` — List images for room (response: list[`ImageResponse`])
- DELETE `/{image_id}` — Hard-delete image (204) — requester must be uploader or have room_service.WRITE
...
- Many endpoints require authentication; several require permission checks (examples: room_service.WRITE, OFFER_MANAGEMENT.WRITE, BOOKING.WRITE, ISSUE_RESOLUTION.WRITE, Admin_Creation.WRITE). Refer to the route function docstrings and the dependencies in `app/dependencies/authentication.py` for exact enforcement.

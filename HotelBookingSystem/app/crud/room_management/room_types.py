...
async def insert_room_type(db: AsyncSession, data: dict) -> RoomTypes:
	room_type_record = RoomTypes(**data)
	db.add(room_type_record)
	await db.flush()
...
async def fetch_room_type_by_id(db: AsyncSession, room_type_id: int) -> Optional[RoomTypes]:
	query_result = await db.execute(select(RoomTypes).where(RoomTypes.room_type_id == room_type_id))
	return query_result.scalars().first()


async def fetch_room_type_by_name(db: AsyncSession, type_name: str) -> Optional[RoomTypes]:
	query_result = await db.execute(select(RoomTypes).where(RoomTypes.type_name == type_name))
	return query_result.scalars().first()
